...
(function ($) {
	...
	var contains = function (lookIn, forWhat) {
		return lookIn.indexOf(forWhat) != -1;
	};
	...
	var generateBlankGpadMap = function (gpadType) {
		...
		for (var axis in padConfigs.axis_dual) {
			blankMap['buttonToRaw']['axis_dual'][axis] = {
				x : false,
				y : false
			};
		}
	...
	var launch_edit_action_map = function (actionMapId) {
		...
		var actionMapPP = function (item) {
			...
			var gamepadOptions = [{
					...
					value : 'false',
					style : 'off',
					message : 'Warning, this will remove all mappings that currently use the gamepad'
				}
			];
			...
			inpoot.utils.styledSelect({
				...
				options : [{
						...
						value : 'false',
						style : 'off',
						message : 'Warning, this will remove all mappings that currently use the keyboard'
					}
				],
				callBack : function (newSelected) {
					thisActionMap.keyboard = newSelected.value;
					saveActionMap();
				}
			});
			...
			inpoot.utils.styledSelect({
				...
				options : [{
						...
						value : 'false',
						style : 'off',
						message : 'Warning, this will remove all mappings that currently use the mouse'
					}
				],
				callBack : function (newSelected) {
					thisActionMap.mouse = newSelected.value;
					saveActionMap();
				}
			});
			...
			inpoot.utils.styledSelect({
				...
				callBack : function (newSelected) {
					thisActionMap.gamepad = newSelected.value;
					saveActionMap();
				}
			});
			...
			var drawMappings = function (actionId) {
				...
				$.tmpl('inpoot.action_map.action_list_mappings', thisActionMap, {
					rendered : drawMapPP
				}).appendTo($('.inpoot-map-form-right-inner').html(''));
			};
			...
			var startLiveMapping = function (item) {
				...
				var displayInput = function (type, inputInfo) {
					...
					$.tmpl('inpoot.action_map.action_edit_action_map_item', {
						name : inputInfo.text
					}, {
						rendered : inputPP
					...
					if (actionMap.keyboard && actionMap.keyboard != "false") {
						...
						for (key in inputsDown['keyboard']) {
							...
							if (inputsDown['keyboard'][key] === true && !CI['keyboard'][key]) {
								addInput('keyboard', {
									text : inpoot.getKeyboardMap(key),
									value : key
								});
				...
				if (actionMap.mouse && actionMap.mouse != "false") {
					...
					mouseArea.find('.inpoot-gather-add-button').click(function () {
						var gpadOptions = [{
								...
								text : 'back button',
								value : 'mouse_button_back'
							}, {
								text : 'forward button',
								value : 'mouse_button_forward'
							}, {
								text : 'scroll forward',
								value : 'mouse_scroll_forward'
							}, {
								text : 'scroll back',
								value : 'mouse_scroll_back'
							}
						];
				...
				if (actionMap.gamepad && actionMap.gamepad != "false") {
					...
					for (var button in mapGamepad.button) {
						addOption(getOptionText(button, mapGamepad.button), button, {
							subType : 'button'
						});
					}
					
					for (var axis in mapGamepad.axis_dual) {
						addOption(getOptionText(axis, mapGamepad.axis_dual) + ' up', axis, {
							buttonType : 'axis_dual',
							subType : 'y',
							direction : 'up'
						});
						addOption(getOptionText(axis, mapGamepad.axis_dual) + ' down', axis, {
							buttonType : 'axis_dual',
							subType : 'y',
							direction : 'down'
						});
						addOption(getOptionText(axis, mapGamepad.axis_dual) + ' left', axis, {
							buttonType : 'axis_dual',
							subType : 'x',
							direction : 'left'
						});
						addOption(getOptionText(axis, mapGamepad.axis_dual) + ' right', axis, {
							buttonType : 'axis_dual',
							subType : 'x',
							direction : 'right'
						});
			...
			var drawLiveMapping = function (actionId, mapId) {
				...
				$.tmpl('inpoot.action_map.action_edit_mapping', {
					actionMap : thisActionMap,
					actionId : actionId,
					mapId : mapId,
					inputCount : inputCount
				}, {
					rendered : startLiveMapping
				}).appendTo($('.inpoot-map-form-right-inner').html(''));
			};
			...
			var editAction = function (actionId) {
				...
				var editActionPP = function (item) {
					...
					var addNewInputCombination = function () {
						...
						$.tmpl('inpoot.action_map.action_edit_action_input_hidden', [{
									id : mapId,
									inputs : []
								}
							], {
							rendered : editActionMapping
						}).appendTo(editNode.find('.inpoot-main-gamepad-edit-action-options-list'));
					};
					...
					$.tmpl('inpoot.action_map.action_edit_action_input', editData.inputs || [], {
						rendered : editActionMapping
					}).appendTo(editNode.find('.inpoot-main-gamepad-edit-action-options-list').html(''));
				};
				
				var templateData = {
					actionId : actionId,
					descr : thisAction.descr,
					inputs : thisAction.inputs
				};
				
				//render the area for editing
				$.tmpl('inpoot.action_map.action_edit_action_inputs', templateData, {
					rendered : editActionPP
				}).appendTo($('.inpoot-map-form-right-inner').html(''));
			...
			var switchToGroup = function (groupName, groupActions) {
				...
				$.tmpl('inpoot.action_map.action_list_item', groupActions, {
					rendered : actionPP
				}).appendTo($('.inpoot-map-form-middle-inner').html(''));
			...
			$.tmpl('inpoot.action_map.action_list', getGroupedActions(thisActionMap.mapping), {
				rendered : mapPP
			}).appendTo($('.inpoot-map-form-left-inner'));
		...
		inpoot.utils.modal.show({
			...
			data : {
				actionMap : thisActionMap,
				gamepads : storedGamepads
			},
			behavior : actionMapPP
		});
	};
	...
	var launch_mappings = function () {
		...
		var renderMappingList = function () {
			...
			listContainer.html('');
			$.tmpl('inpoot.mapping-list', allActionMappings, {
				rendered : mapListPP
				...
				getButtonPress(function (button) {
					...
					if (thisButton.type == 'button') {
						...
						if (thisButton.style == "up" || thisButton.style == "down") {
							...
							thisGMap['rawToButton']['axesMap'][thisButton.bid + 'down'] = {
								bid : thisButton.bid,
								index : button.number,
								subType : 'y',
								direction : 'down'
							};
							thisGMap['rawToButton']['axesMap'][thisButton.bid + 'up'] = {
								bid : thisButton.bid,
								index : button.number,
								subType : 'y',
								direction : 'up'
							...
							thisGMap['rawToButton']['axesMap'][thisButton.bid + 'left'] = {
								bid : thisButton.bid,
								index : button.number,
								subType : 'x',
								direction : 'left'
							};
							thisGMap['rawToButton']['axesMap'][thisButton.bid + 'right'] = {
								bid : thisButton.bid,
								index : button.number,
								subType : 'x',
								direction : 'right'
					...
					thisGMap['rawToButton'][rawType][button.number] = {
		...
		var gpadPP = function (item) {
			...
			var addButtonBehavior = function (button, thisButton, btype) {
				...
				newButton.hover(function () {
					$('.gamepadclass-' + button).addClass('pressed');
				}, function () {
					$('.gamepadclass-' + button).removeClass('pressed');
				});
	...
	var launch_calibrate = function () {
		...
		var redrawGamepads = function (displayList) {
			...
			$.tmpl('inpoot.gamepad-gamepad', {
				gamepads : displayList,
				gamepadPP : gamepadPP
			}).appendTo(mainNode.find('.inpoot-gamepad-list'));
		};
	...
	var launch_players = function () {
		...
		var drawPlayers = function () {
			...
			//render the players
			$.tmpl('inpoot.player', inpoot.players, {
				rendered : eachPlayerPP
			}).appendTo(globalUINode.find('.inpoot-players-list').html(''));
		};
			...
			uiNode.find('.inpoot-player-gamepad-index').click(function () {
				...
				for (var i = 0; i < 4; i++) {
					...
					if (gpadList[i]) {
						...
						gpadIndexes.push({
							value : i,
							text : 'Gamepad Slot ' + (i + 1) + " : " + gpadList[i].gpadType,
							disabled : disabled
						});
			...
			var showPlayerOptions = function () {
				...
				if (item.data.gamepad && item.data.gamepad != "false") {
					...
					if (axisList.length > 0) {
						...
						//append the title
						$.tmpl('inpoot.player-options-title', {
							title : 'Invert Gamepad Y Axes'
						}).appendTo(optionsNode);
						
						//append the options
						$.tmpl('inpoot.player-invert-axis', axisList, {
							rendered : axisPP
						}).appendTo(optionsNode);
					}
				}
				...
				if (optionSets === 0) {
					$.tmpl('inpoot.player-options-title', {
						title : 'No additional options for this player'
					}).appendTo(optionsNode);
				}
